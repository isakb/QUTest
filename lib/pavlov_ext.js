(function() {
    ...
    // node-coverage
    if (window.$$_l) {
        // NB. window.QUnit.done cannot be trusted with async tests
        window.QUnit.moduleStart = function() {
            moduleCount++;
            window.clearTimeout(nodeCoverageTimeout);
        };
        window.QUnit.moduleDone = function() {
            moduleCount--;
            if (moduleCount === 0) {
                nodeCoverageTimeout = window.setTimeout(function() {
                    window.$$_l.submit('qutest');
                    window.setTimeout(function() {
                        window.console.log('_pjs_exit');
                    }, 2500);
                }, 1000);
            }
        };
        window.console.log('_pjs_wait');
    }
})();
